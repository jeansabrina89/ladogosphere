create or replace function public.retourner_vente(
  p_vente_id uuid, p_cle_idempotence text, p_lignes jsonb, p_total numeric,
  p_arrondi numeric, p_ecriture_lignes jsonb, p_motif text, p_user_id uuid)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  -- Le taux de la vente d'origine voyage avec la ligne rendue : un retour se
  -- ventile aux taux de la pièce qu'il corrige, jamais à ceux d'aujourd'hui.
