-- Le paiement par avoir depuis l'espace client ne laissait aucune trace dans
-- paiements_resa, contrairement au chemin admin (payerParAvoir). Consequence :
-- synchroniserComptaResa ne voyait pas le mouvement et n'imputait pas la
-- contrepartie avoir (compte 2035). On insere desormais la ligne dans la meme
-- transaction que la consommation de l'avoir (tout-ou-rien).
