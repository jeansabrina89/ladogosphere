-- Les deux RPC de caisse écrivaient « taux_tva = 0 » en dur sur la facture du
-- client : un achat au comptoir porté sur facture perdait sa TVA en chemin.
-- Le taux et le secteur voyagent maintenant avec la ligne, figés à la vente.
create or replace function public.finaliser_vente(
  p_cle_idempotence text, p_canal text, p_client_id uuid, p_mode text,
  p_lignes jsonb, p_total numeric, p_arrondi numeric, p_ecriture_lignes jsonb,
  p_facture_id uuid, p_user_id uuid, p_montant_recu numeric default null)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  insert into public.ventes_lignes
    (vente_id, article_id, libelle, quantite, prix_unitaire, taux_tva, montant, secteur_tdfn)
  ...
  if p_facture_id is not null then
    ...
    insert into public.facture_lignes
      (facture_id, ordre, libelle, quantite, prix_unitaire, compte_produit, taux_tva, secteur_tdfn)
