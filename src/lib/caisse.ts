...
export async function articlesVendables(): Promise<ArticleVendable[]> {
  const { data } = await supabaseAdmin
    .from("articles")
    .select("id, nom, reference, code_barres, prix_vente, taux_tva, motif_tva, secteur_tdfn, stock_actuel, stock_reserve, unite, photo_path, type_article, categorie, remise_membre_exclue, date_limite, statut_vitrine")
    .eq("actif", true)
