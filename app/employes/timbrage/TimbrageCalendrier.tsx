...
function getBadge(
  ...
  // Aucun timbrage
  if (j && j.du > 0) return { label: "À compléter", bg: "#FEF3C7", text: "#D97706" };
  if (statut === "repos" || statut === "repos_vacances") return { label: "Repos", bg: "#F1F5F9", text: "#9CA3AF" };
  if (statut === "vacances") return { label: "Vacances", bg: "#FEF9C3", text: "#CA8A04" };
  return null;
}
...
function FormPanel({
  ...
  const handleSave = async () => {
    setLoading(true);
    setErreur(null);
    const base = { employe_id: empId, date, note: note || null };
    const body = mode === "travail"
      ? { ...base, heure_debut_matin: hdm, heure_fin_matin: hfm,
          heure_debut_aprem: hda, heure_fin_aprem: hfa,
          type_absence: null,
          ...(showValideAdmin ? { valide_admin: valideAdmin } : {}) }
      : { ...base, heure_debut_matin: null, heure_fin_matin: null,
...
export default function TimbrageCalendrier({
  ...
  // Dériver les paramètres de date depuis la prop "mois"
  const moisPad = mois.split("-")[1];
  const annee   = parseInt(mois.split("-")[0]);
  const moisNum = parseInt(moisPad);

  // Aujourd'hui côté client (heure locale)
  const dNow = new Date();
  const aujourd_hui = [
    dNow.getFullYear(),
  ...
  decompteJours.forEach(j => { decompteParDate[j.date] = j; });

  // Grille calendrier
  const nbJours     = new Date(annee, moisNum, 0).getDate();
  const premierJour = new Date(`${mois}-01T12:00:00`).getDay();
  const offset      = premierJour === 0 ? 6 : premierJour - 1; // lundi = 0

  // Peut-on ouvrir cette case ?
  const peutOuvrir = (t: TimbrageRow | null, estFutur: boolean): boolean => {
    if (mode === "employe") {
      if (estFutur) return false;
      if (t?.valide_admin === true) return false;
    }
    return true;
  };
  ...
  return (
    <div>
          ...
          <div style={{ minWidth: "480px" }}>
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {Array.from({ length: nbJours }, (_, i) => {
                const jour = i + 1;
                const dateStr = `${mois}-${String(jour).padStart(2, "0")}`;
                ...
                const estVerrouille = mode === "employe" && t?.valide_admin === true;
                const ouvrable      = peutOuvrir(t, estFutur);

                let bg = bgCaseStatut(statut, estWeekend);
                if (estSelectionne) bg = "#E8F5F4";
                else if (estVerrouille) bg = "#F8FAFC";
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      if (!ouvrable) return;
                      setSelectedDate(estSelectionne ? null : dateStr);
                    }}
                    className={`rounded-xl p-2 flex flex-col transition-all select-none${ouvrable ? " cursor-pointer hover:shadow-md" : " cursor-default"}`}
                    style={{
                      backgroundColor: bg,
                      border: estSelectionne ? "2px solid #4AAEA0" : estAujourdhui ? "2px solid #64748B" : "1px solid #E2E8F0",
                      opacity: estFutur ? 0.6 : estVerrouille ? 0.7 : 1,
                      minHeight: "80px",
                    }}
                  >
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <div className="flex gap-0.5">
                        {mode === "admin"   && t?.valide_admin === false && (
                          <span style={{ fontSize: "10px", color: "#F97316" }} title="Non validé">⚠</span>
                        )}
                        {mode === "employe" && estVerrouille && (
                          <span style={{ fontSize: "10px", color: "#9CA3AF" }} title="Validé — lecture seule">🔒</span>
                        )}
                      </div>
                    </div>
                    ...
                    {statut && (
                      <span style={{
                        ...
                        backgroundColor: bgPillStatut(statut), color: textPillStatut(statut),
      ...
      <div className="flex flex-wrap gap-2 mt-3 px-1 items-center">
        ...
        {mode === "admin"   && <span className="text-xs text-orange-400 font-semibold">⚠ = non validé</span>}
        {mode === "employe" && <span className="text-xs text-gray-400">🔒 = validé (lecture seule)</span>}
        <span className="text-xs text-gray-400 ml-1">
          {mode === "employe" ? "Cliquez une case pour pointer vos heures" : "Cliquez une case pour modifier"}
        </span>
      </div>
