...
export default async function DepensePage({
  ...
  // Entrer en stock est un geste de gestion, pas de comptoir — et la
  // permission dépend de la CATÉGORIE : « Matières de fabrication » demande
  // l'atelier, « Marchandises à revendre » la gestion boutique.
  const perimetreDepense = perimetreDuCompte(depense.compte_charge);
  const peutEntrer =
    avecStock &&
    depense.statut !== "annulee" &&
    !!perimetreDepense &&
    accesStockAccorde(acces.permissions, perimetreDepense, "gestion");

  const articlesEntree = peutEntrer
    ? catalogue
        .filter((a) => a.actif)
        // La liste reste mixte quand on a droit aux deux — une puce NFC
        // achetée sur une facture de marchandises doit rester atteignable —
        // mais on ne montre jamais un périmètre auquel on n'a pas accès.
