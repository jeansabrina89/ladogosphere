...
export default function GestionAvoir({
  ...
  const mouvementsFiltres = voirTout
    ? mouvements
    : mouvements.filter(m => m.type === "ajout_manuel");

  const aNonAjoutManuel = mouvements.some(m => m.type !== "ajout_manuel");
      ...
      {mouvements.length > 0 ? (
        <div className="space-y-1">
          ...
          {mouvementsFiltres.length === 0 ? (
            <p className="text-gray-400 text-sm">Aucun ajout manuel.</p>
          ) : (
          ...
          {aNonAjoutManuel && (
            <button
              type="button"
              onClick={() => setVoirTout(!voirTout)}
              className="mt-2 text-xs font-semibold px-3 py-1.5 rounded-lg border transition"
              style={{ borderColor: "#E2E8F0", color: "#6B7280", backgroundColor: "white" }}>
              {voirTout
                ? "N'afficher que les ajouts"
                : `Voir tous les mouvements (${mouvements.length})`}
            </button>
          )}
        </div>
      ) : (
